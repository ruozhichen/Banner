...
Banner2:
1.实现图片的缩放，图片大小原来283*178，现如今picture1改为200*125,picture2改为400*251。样例中用的是picture1的大小
2.将标题顶端与图片顶端对齐。
并且修正了当单词右边不存在图片，单词会一直往右边填写甚至超出背景区域的问题
*/
using namespace std;
const int maxn=100;
...
//printf("%d %d\n",left,right);
    ...
    textInBg_y=picInBg_y+imageTopPixel+SFontWholeHeight; //注意标题位置参照是以字母最左下角
