                      ...
                      LPARAM lParam)
{
    ...
    case WM_COMMAND:/*单击事件，就是触发按钮的事件*/
    {
        switch(LOWORD(wParam))
        {
            case IDC_BUTTON1:{
                printf("按钮被点击\n");
                //SetWindowText(buttonImage,TEXT("我的button1"));
            ...
            case IDC_BUTTON2:{
                printf("按钮被点击\n");
                //SetWindowText(buttonTitle,TEXT("我的button2"));
            ...
            case IDC_BUTTON3:{
                printf("按钮被点击\n");
                //SetWindowText(buttonExplain,TEXT("我的button3"));
...
printf( "The current directory is:%s ",buffer);  //buffer会变化！！！
                /*
                这里可以显示，也可以保存，但是一旦之前单击过choose按钮，则显示的是灰框。
                原因：
                如果把input文件放到“素材”文件里面，那么单击choose按钮后，仍然可以显示图片。
                因为选择的对象是在“素材”中，选择过后，当前目录就变成“素材”文件了！！！
                */
                IplImage* img0 = cvLoadImage("./input/picture.jpg");
                cvShowImage("Asda",img0);
                cvSaveImage("./input/asda.jpg",img0);
                break;
            }
            case IDC_BUTTONSTART:{

    /*
    //一旦先单击choose按钮，再单击此按钮的话，相对目录就不管用了
    //txt、图片都不行
    FILE *f1;
    int tmp;
    ...
    IplImage* img0 = cvLoadImage("input/picture.jpg");
    cvShowImage("Asda",img0);
    cvSaveImage("input/asda11.jpg",img0);
    */
