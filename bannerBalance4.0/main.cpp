...
#define EXPLAIN 2
#define KIND 3
#define titleSpaceWidthScaleHor 0.25
#define titleSpaceHeightScaleHor 0.35
#define titleSpaceWidthScaleVer 0.8
#define titleSpaceHeightScaleVer 0.25
...
n为元素个数
*/
void init(Element*current,char*filename,int sizes,Space titleSpace){
    char imgName[maxn];
    char tmpstr[maxn];
...
初始化布局
*/
void initLayout(Layout &layout,int ith,int itv,int ieh,int iev,int teh,int tev){
...
int main()
{
	...
	Space whiteSpace,originalWhiteSpace,interestBox,titleSpace;
    ...
    */
    ...
    ///titleSpace.w=BannerWidth*titleSpaceWidthScaleHor;titleSpace.h=BannerHeight*titleSpaceHeightScaleHor;
    titleSpace.w=BannerWidth*titleSpaceWidthScaleVer;titleSpace.h=BannerHeight*titleSpaceHeightScaleVer;
    ...
    initLayout(horLayout,HORIZONTALRIGHT,NOMATTER,HORIZONTALRIGHT,NOMATTER,NOMATTER,VERTICALUP);
    initLayout(verLayout,NOMATTER,VERTICALBOTTOM,NOMATTER,VERTICALBOTTOM,NOMATTER,VERTICALUP);
    init(currentSolution,filename,sizes,titleSpace);
...
//printf("%d\n",orientation);
		...
		for(int i = 0; i < l(t); i++){
			...
			///chooseNeighbour(candidateSolution, whiteSpace, titleSpace, sizes, t, horLayout);
			chooseNeighbour(candidateSolution, whiteSpace, titleSpace, sizes, t, verLayout);
			//元素移动后的cost减去之前的cost
...
printf("%s %d %d\n",bestSolution[i].name,bestSolution[i].x_pos,bestSolution[i].y_pos);
    ...
    save_bitmap(bg,"result6.jpg",1);

    FILE*f2;
    f2=fopen("output/result6.txt","w");
    fprintf(f2,"cost:\t%f\nBalanceCost:\t%f\nAlignmentCost:\t%f\n",cost(bestSolution, whiteSpace, interestBox, sizes, designType),BannerBalanceCost,BannerAlignmentCost);
    fclose(f2);
	return 0;
}
