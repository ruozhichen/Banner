...
/**
能根据一定的cost来计算广告的平衡，并由此产生cost最小的广告布局
cost的计算根据论文《Gaudi An Automated Graphic Design Expert Syste》里提供的代码
但是感觉里面在balanceCost的计算不是很好，所以结果不是很理想，也有可能是我weight的计算不是很好吧。
*/
using namespace std;
const int maxn=100;
...
int checkBorders(Element g, Space *space, int size);
...
//printf("3\n");
        ...
        current[i].x_pos=rand()%(BannerWidth-current[i].width);
        current[i].y_pos=rand()%(BannerHeight-current[i].height);
...
printf("%d %d\n",current[i].x_pos,current[i].y_pos);
//printf("4\n");
    }
    fclose(f1);
}
...
int main()
{
	...
	int designType, orientation, keepChecking=TRUE;
...
printf("size:%d\n",sizes);
	for(int i=0;i<sizes;i++){
		if(bestSolution[i].normalGroup == 1){
			printf("%d %d\n", bestSolution[i].x_pos,bestSolution[i].y_pos);
		}
	}

	/*Cost*/
	printf("cost:%f \n", cost(bestSolution, whiteSpace, interestBox, sizes, designType));
	/*Size modifications*/
    ...
    //创建白色的背景图
    CvSize bgSize;
    bgSize.width=whiteSpace.w;
    bgSize.height=whiteSpace.h;
...
float cost(Element *candidate, Space space, Space interestBox, int size, int designType){
	...
	/*If not...*/
		...
		return ((balanceCost*BALANCE_PERCENTAGE)+(alignmentCost*ALIGNMENT_PERCENTAGE)); //平衡占30%，对齐占70%
...
//printf("%f %f %f %d %d\n",weights,xWeights,yWeights,centroidX,centroidY);
	...
	maxDistance = sqrt(pow(space.w, 2) + pow(space.h, 2))/2;
