...
所有元素的整体“重心”坐标计算改成加权平均，即权重*坐标的累加和/权重的累加和，效果比原来要好很多。

对space扩大的方式也进行了一些修改，根据各元素竖直or水平超出space的个数，来对应的扩大space的高or宽
（虽然目前还没用到space的扩大）
*/
using namespace std;
const int maxn=100;
...
printf("weight:%f x:%d y:%d\n",current[i].weight,current[i].x_pos,current[i].y_pos);
//printf("4\n");
    }
    fclose(f1);
}
...
printf("%s %d %d\n",bestSolution[i].name,bestSolution[i].x_pos,bestSolution[i].y_pos);
    ...
    save_bitmap(bg,"result5.jpg",1);

    FILE*f2;
    f2=fopen("output/result5.txt","w");
    fprintf(f2,"cost:\t%f\nBalanceCost:\t%f\nAlignmentCost:\t%f\n",cost(bestSolution, whiteSpace, interestBox, sizes, designType),BannerBalanceCost,BannerAlignmentCost);
    fclose(f2);
	return 0;
}
